cat <<EOF > main.js
function showTab(id) {
    document.querySelectorAll('.module').forEach(m => m.classList.remove('active'));
    document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
    document.getElementById(id).classList.add('active');
    event.currentTarget.classList.add('active');
}

const canvas = document.getElementById('z-canvas');
const ctx = canvas.getContext('2d');
const size = Math.min(window.innerWidth, window.innerHeight) * 0.75;
canvas.width = size; canvas.height = size;

let bots = [];
const CORNER_ZONE = 20; 

class CornerBot {
    constructor(vy) {
        this.x = size / 2; this.y = size / 2;
        this.vx = 5; this.vy = vy;
        this.history = [];
        this.hBounces = 0; this.vBounces = 0;
    }

    update() {
        this.x += this.vx; this.y += this.vy;

        if (this.x <= 0 || this.x >= size) { this.vx *= -1; this.hBounces++; }
        if (this.y <= 0 || this.y >= size) { this.vy *= -1; this.vBounces++; }

        const corners = [{x:0,y:0}, {x:size,y:0}, {x:0,y:size}, {x:size,y:size}];
        corners.forEach(c => {
            let d = Math.sqrt((this.x - c.x)**2 + (this.y - c.y)**2);
            if (d < CORNER_ZONE) {
                document.getElementById('prec').innerText = (1 - d/CORNER_ZONE).toFixed(6);
            }
        });

        document.getElementById('ratio').innerText = (this.hBounces / (this.vBounces || 1)).toFixed(6);

        this.history.push({x: this.x, y: this.y});
        if (this.history.length > 250) this.history.shift();
    }

    draw() {
        ctx.strokeStyle = '#0f0'; ctx.lineWidth = 1; ctx.beginPath();
        this.history.forEach((p, i) => i === 0 ? ctx.moveTo(p.x, p.y) : ctx.lineTo(p.x, p.y));
        ctx.stroke();
        ctx.fillStyle = '#fff'; ctx.fillRect(this.x-3, this.y-3, 6, 6);
    }
}

window.launchBot = () => {
    bots.push(new CornerBot(parseFloat(document.getElementById('vy-in').value)));
};

function loop() {
    ctx.fillStyle = 'rgba(0,0,0,0.1)'; ctx.fillRect(0,0,size,size);
    bots.forEach(b => { b.update(); b.draw(); });
    requestAnimationFrame(loop);
}
loop();
EOF