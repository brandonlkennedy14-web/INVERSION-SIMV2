...
window.onload = () => {
    window.downloadData = () => {
        // Now downloads both the Euclidean Voids (with Topology) AND the Riemann Zeros
        const exportData = {
        ...
        dl.setAttribute("download", "verified_topology_and_zeta.json");
    ...
    cp.height = cb.height = ch.height = cHand.height = cZeta.height = 700; 
    ...
    let s = { x: 250, y: 350, vx: 5, vy: 7, bounces: 0, scanned: 0, found: 0, shotsFired: 0, recent: [], currentBot: 0, mapMode: 0, currentLoss: Infinity };
    let replay = { active: false, x: 250, y: 350, vx: 5, vy: 7, bounces: 0, targetBounces: 0, name: "", trail: [] };
    ...
    cb.addEventListener('click', (e) => {
        ...
        s.recent.forEach((item) => {
            ...
            let y = s.mapMode === 0 ? (item.bounces * 2) % 700 : (item.vy * 50) % 700;
            let dist = Math.sqrt((clickX - x)**2 + (clickY - y)**2);
            if(dist < minDist) { minDist = dist; clickedNode = item; }
        });
        if(clickedNode) {
            replay.active = true; replay.x = 250; replay.y = 350; replay.vx = clickedNode.vx; replay.vy = clickedNode.vy;
            replay.bounces = 0; replay.targetBounces = clickedNode.bounces; replay.name = clickedNode.seq; replay.trail = []; 
            window.tab('sim'); ctxP.fillStyle = '#000'; ctxP.fillRect(0,0,500,700); 
        } else { s.mapMode = (s.mapMode + 1) % 2; window.renderSymmetryMap(); }
    });

    function simulateLoss(testVy, botType) {
        let tx = 250, ty = 350, tvx = 5, tvy = testVy, bCount = 0, mLoss = Infinity;
        for(let i=0; i<30000; i++) {
            tx += tvx; ty += tvy;
            let hitX = (tx <= 0 || tx >= 500); let hitY = (ty <= 0 || ty >= 700);
            if(hitX) { tvx *= -1; tx = (tx <= 0) ? 0 : 500; bCount++; }
            if(hitY) { tvy *= -1; ty = (ty <= 0) ? 0 : 700; bCount++; }
            if(hitX || hitY) {
                let dx = Math.min(tx, 500-tx); let dy = Math.min(ty, 700-ty);
    ...
    function calcGhost() {
        let gx = 250, gy = 350, gvx = s.vx, gvy = s.vy; let pts = [{x: gx, y: gy}];
        for(let k=0; k<10; k++) {
            let tx = gvx > 0 ? (500 - gx)/gvx : (0 - gx)/gvx; let ty = gvy > 0 ? (700 - gy)/gvy : (0 - gy)/gvy;
    ...
    function reseed() {
        ...
        s.x = 250; s.y = 350; s.bounces = 0; s.ghostPts = calcGhost();
    }
    ...
    // 2. THE RIEMANN ZETA ENGINE (UPGRADED TO HUNT & VERIFY TO 100)
    let verifiedZeros = [];
    ...
    window.renderZeta = () => {
        ctxZ.fillStyle = 'rgba(0,0,0,0.5)'; ctxZ.fillRect(0,0,500,700);
        ...
        for(let y=0; y<=700; y+=2) {
            let t = (y / 700) * 100; // Expanded map to t=100
            let mag = calcZetaMagnitude(t);
            let x = 250 + (mag * 60); 
            if(y===0) ctxZ.moveTo(x, y); else ctxZ.lineTo(x, y);
        }
        ctxZ.stroke();
        ctxZ.strokeStyle = 'rgba(0, 255, 255, 0.2)'; ctxZ.beginPath(); ctxZ.moveTo(250, 0); ctxZ.lineTo(250, 700); ctxZ.stroke();
        ...
        zetaBots.forEach(b => {
            ...
            // AUTOMATED VERIFICATION: If the gradient zeroes out in a deep valley, lock it.
            if(Math.abs(gradient) < 0.0005 && L1 < 0.05) {
                if(!verifiedZeros.some(z => Math.abs(z - b.t) < 0.5)) {
                    verifiedZeros.push(b.t);
                    b.t += 3.5 + Math.random(); // JUMP FORWARD TO HUNT NEXT
                }
            } else {
                b.t = b.t - (gradient * 0.01); // Slide faster
                if(b.t < 0) b.t = 0; if(b.t > 100) b.t = 100;
            }

            let y = (b.t / 100) * 700; let x = 250 + (calcZetaMagnitude(b.t) * 60);
    ...
    function project3D(px, py, pz) {
        ...
        let y1 = py * Math.cos(rotX) - z1 * Math.sin(rotX); return { x: 250 + (x1 * 140), y: 350 + (y1 * 140) };
    }

    window.renderHyper = () => {
        ctxH.fillStyle = 'rgba(0,0,0,0.4)'; ctxH.fillRect(0,0,500,700); 
        let R = 1.2; let r = 0.5; 
        ctxH.strokeStyle = 'rgba(255, 0, 0, 0.4)'; ctxH.lineWidth = 1; ctxH.beginPath();
        for(let i=0; i<s.ghostPts.length; i++) {
            let u = (s.ghostPts[i].x / 500) * Math.PI * 2; let v = (s.ghostPts[i].y / 700) * Math.PI * 2; 
        ...
        // WRAP THE ZETA LINE AROUND THE TORUS
        ctxH.strokeStyle = 'rgba(255, 255, 255, 0.8)'; ctxH.lineWidth = 2; ctxH.beginPath();
        for(let t=0; t<=100; t+=0.2) {
            let u = (t / 100) * Math.PI * 4; // Wrap it twice around the Torus
            let mag = calcZetaMagnitude(t); let v = 0; 
            let warpedR = R + (mag * 0.6); 
            let px = (warpedR + r * Math.cos(v)) * Math.cos(u); let py = (warpedR + r * Math.cos(v)) * Math.sin(u); let pz = r * Math.sin(v);
            let pt = project3D(px, py, pz);
            if(t===0) ctxH.moveTo(pt.x, pt.y); else ctxH.lineTo(pt.x, pt.y);
        }
        ctxH.stroke();

        // DRAW VERIFIED ZETA ZEROS AS GOLD RINGS ON THE DONUT
        verifiedZeros.forEach(z => {
    ...
    window.renderHands = () => {
        ctxHand.fillStyle = '#000'; ctxHand.fillRect(0,0,500,700);
        ...
        ctxHand.strokeStyle = 'rgba(255, 255, 255, 0.2)'; ctxHand.strokeRect(10, 210, 480, 450);
        
        let maxSpike = Math.max(...harmonics.slice(1), 1);
        ctxHand.beginPath();
        for(let i = 1; i <= 1500; i++) {
            if(harmonics[i] > 0) {
                let height = (harmonics[i] / maxSpike) * 440; let x = 10 + (i / 1500) * 480;
                ctxHand.fillStyle = `rgb(${getFrequencyColor(i)})`; ctxHand.fillRect(x, 660 - height, 2, height);
            }
        }
    };

    window.renderSymmetryMap = () => {
        ctxB.fillStyle = '#000'; ctxB.fillRect(0,0,500,700);
        ...
        for(let i=0; i<=500; i+=50) { ctxB.beginPath(); ctxB.moveTo(i, 0); ctxB.lineTo(i, 700); ctxB.stroke(); }
        for(let i=0; i<=700; i+=70) { ctxB.beginPath(); ctxB.moveTo(0, i); ctxB.lineTo(500, i); ctxB.stroke(); }
        ...
        for(let i=0; i<sorted.length-1; i++) {
            if(Math.abs(sorted[i].vy - sorted[i+1].vy) < 0.05) { 
                ...
                let y1 = s.mapMode === 0 ? (sorted[i].bounces * 2) % 700 : (sorted[i].vy * 50) % 700;
                let x2 = s.mapMode === 0 ? ((sorted[i+1].vy - 5) * 100) % 500 : (sorted[i+1].bounces * 5) % 500;
                let y2 = s.mapMode === 0 ? (sorted[i+1].bounces * 2) % 700 : (sorted[i+1].vy * 50) % 700;
        ...
        if(replay.trail.length > 0) {
            ...
            replay.trail.forEach(b => {
                ...
                let y = s.mapMode === 0 ? (b * 2) % 700 : (replay.vy * 50) % 700;
    ...
    function engine() {
        if(replay.active) {
            for(let i=0; i<30; i++) { 
                ...
                let hitX = (replay.x <= 0 || replay.x >= 500); let hitY = (replay.y <= 0 || replay.y >= 700);
                if(hitX) { replay.vx *= -1; replay.x = replay.x <= 0 ? 0 : 500; replay.bounces++; replay.trail.push(replay.bounces); }
                if(hitY) { replay.vy *= -1; replay.y = replay.y <= 0 ? 0 : 700; replay.bounces++; replay.trail.push(replay.bounces); }
            ...
            ctxP.fillStyle = 'rgba(0,0,0,0.02)'; ctxP.fillRect(0,0,500,700);
            ...
            for(let i = 0; i < 1000; i++) {
                ...
                let hitX = (s.x <= 0 || s.x >= 500); let hitY = (s.y <= 0 || s.y >= 700);

                if(hitX) { s.vx *= -1; s.x = (s.x <= 0) ? 0 : 500; s.bounces++; }
                if(hitY) { s.vy *= -1; s.y = (s.y <= 0) ? 0 : 700; s.bounces++; }

                if(hitX || hitY) {
                    let dx = Math.min(s.x, 500-s.x); let dy = Math.min(s.y, 700-s.y);
                    let dist = Math.sqrt(dx*dx + dy*dy);
                ...
                if(s.bounces > 1500) {
                    ...
                    if(activeBot.type === 'blade') {
                        ...
                        if(!s.recent.some(r => r.seq === sig)) {
                            ...
                            // CALCULATE TOPOLOGY ON THE FLY
                            let topMatch = getTopology(Math.abs(s.vy));
                            
                            // BURN IT INTO JSON MEMORY
            ...
            ctxP.fillStyle = 'rgba(0,0,0,0.3)'; ctxP.fillRect(0,0,500,700);
            ctxP.strokeStyle = 'rgba(255, 255, 255, 0.15)'; ctxP.lineWidth = 1; ctxP.beginPath();
            for(let i=0; i<=500; i+=50) { ctxP.moveTo(i, 0); ctxP.lineTo(i, 700); }
            for(let i=0; i<=700; i+=70) { ctxP.moveTo(0, i); ctxP.lineTo(500, i); }
            ctxP.stroke();
            ...
            if(gridPulse.alpha > 0.01) {
                ...
                for(let i=0; i<=500; i+=50) { 
                    for(let j=0; j<=700; j+=20) {
                        let dist = Math.sqrt(Math.pow(i-250, 2) + Math.pow(j-350, 2));
